...
(function () {
    ...
    /* ---------- Cookie banner (informativo, no profilazione) ---------- */
    var banner = document.getElementById("cookie-banner");
    if (banner) {
        try {
            if (!localStorage.getItem("csd-cookie-ack")) banner.hidden = false;
        } catch (_) { banner.hidden = false; }
        var okBtn = document.getElementById("cookie-ok");
        if (okBtn) okBtn.addEventListener("click", function () {
            try { localStorage.setItem("csd-cookie-ack", "1"); } catch (_) {}
            banner.hidden = true;
        });
    }
    ...
    if (form) {
        ...
        form.addEventListener("submit", async function (e) {
            ...
            var oggetto = (data.get("oggetto") || "").toString().trim();
            var messaggio = (data.get("messaggio") || "").toString().trim();
            var privacy = data.get("privacy");

            if (nome.length < 2 || nome.length > 80) return showErr("Inserisci un nome valido (2–80 caratteri).");
            if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return showErr("Email non valida.");
            if (oggetto.length < 2 || oggetto.length > 120) return showErr("Oggetto non valido.");
            if (messaggio.length < 10 || messaggio.length > 2000) return showErr("Il messaggio deve avere tra 10 e 2000 caratteri.");
            if (!privacy) return showErr("Devi accettare l'informativa sulla privacy per inviare il messaggio.");
            ...
            try {
                ...
                if (json.success) {
                    ...
                    status.textContent = "Messaggio inviato correttamente. Ti risponderemo il prima possibile.";
                    form.reset();
                } else {
                    showErr("Invio non riuscito: " + (json.message || "riprova più tardi."));
                }
            } catch (err) {
                showErr("Errore di rete. Riprova più tardi oppure scrivi direttamente a asd-centrostudidanza@tiscali.it");
            }
